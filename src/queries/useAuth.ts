...
export function useInviteUser() {
  return useMutation({
    mutationFn: ({ data, accessToken }: { data: InviteUserRequest; accessToken: string }) =>
      inviteUserApi(data, accessToken),
  });
}
...
export function useAuthState() {
  ...
  const accessToken = isClient ? getStoredAccessToken() : null;
