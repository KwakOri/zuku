...
import { useCombinedTeacherSchedule } from "@/queries/useCombinedSchedule";
...
export default function CombinedTeacherSchedule() {
  ...
  // 선생님 데이터가 로드되면 전체 선택으로 초기화
  useMemo(() => {
    if (teachersWithSchedules.length > 0 && selectedTeachers.length === 0) {
      setSelectedTeachers(teachersWithSchedules.map((t) => t.id));
    }
  }, [teachersWithSchedules, selectedTeachers.length]);
  ...
  const teacherData = useMemo(() => {
    const filteredTeachers = teachersWithSchedules
      .filter((t) => {
        if (!selectedTeachers.includes(t.id)) return false;
        ...
        const events: TimelineEvent[] = teacher.classes.flatMap((classItem) => {
          return classItem.class_compositions
            ...
            .map((comp) => {
              ...
              return {
                id: `teacher-${teacher.id}-class-${classItem.id}-${comp.id}`,
    ...
    const sortedTeachers = [...filteredTeachers].sort((a, b) => {
      const compareValue = a.name.localeCompare(b.name, "ko-KR");
    ...
    return (
      <div className="flex items-center justify-center w-full overflow-auto rounded-lg grow bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">선생님 데이터를 불러오는 중...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col w-full h-full rounded-lg grow bg-gray-50">
      ...
      <div className="p-4 bg-white border-b border-gray-200">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              onClick={handleSort}
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm transition-colors bg-primary-100 text-primary-700 font-medium"
            >
              <span>이름</span>
      ...
      <div className="flex-1 overflow-auto scrollbar-hide">
        ...
        >
            ...
            daysOfWeek.map((day, dayIndex) => {
              ...
              return Array.from({ length: dayDurationHours }).map(
                (_, hourIndex) => {
                  ...
                  return (
                    <div
                      key={`grid-${teacher.id}-${dayIndex}-${hourIndex}`}
                      className={`${
            ...
            daysOfWeek.map((day, dayIndex) => {
              ...
              return Array.from({ length: dayDurationHours }).map(
                (_, hourIndex) => {
                  ...
                  return (
                    <div
                      key={`hour-line-${teacher.id}-${dayIndex}-${hourIndex}`}
          ...
          {/* Teacher Rows */}
          {teacherData.map((teacher, rowIndex) => {
            const eventGroups = groupOverlappingEvents(teacher.events);
            ...
            return (
              <Fragment key={teacher.id}>
                ...
                {eventGroups.map((eventGroup, groupIndex) => {
                  ...
                  return (
                    <div
                      key={`group-${teacher.id}-${groupIndex}`}
                      style={{
                        ...position,
                        gridRow: rowIndex + 3,
                        height: "50px",
