...
interface ClassScheduleCanvasProps {
  schedules: StudentComprehensiveSchedule[];
  className?: string;
}
...
export default function ClassScheduleCanvas({ schedules, className = '' }: ClassScheduleCanvasProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);

  // Canvas 크기 계산
  const canvasSize = useMemo(() => {
    const totalDaySlots = SCHEDULE_CONFIG.dayNames.reduce((acc, _, dayIndex) => {
    ...
    return {
      width: CANVAS_CONFIG.studentInfoWidth + totalDaySlots * CANVAS_CONFIG.slotWidth,
      height: CANVAS_CONFIG.headerHeight + schedules.length * CANVAS_CONFIG.rowHeight
    };
  }, [schedules.length]);
  ...
  const drawHeader = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
    ...
    SCHEDULE_CONFIG.dayNames.forEach((dayName, dayIndex) => {
      ...
      for (let hour = startHour; hour <= endHour; hour++) {
        const hourX = timeX + (hour - startHour) * SCHEDULE_CONFIG.slotsPerHour * CANVAS_CONFIG.slotWidth;
  ...
  // 학생 행 그리기
  const drawStudentRow = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, schedule: StudentComprehensiveSchedule, rowIndex: number) => {
    ...
    const studentData = [student.id, student.name, schedule.school, student.grade];
    const studentWidths = [60, 80, 100, 60];
    let currentX = 0;
    ...
    studentData.forEach((data, index) => {
      ctx.fillText(data.toString(), currentX + studentWidths[index] / 2, y + CANVAS_CONFIG.rowHeight / 2 + 5);
      currentX += studentWidths[index];
    });
    ...
    schedule.weeklySchedule.forEach((daySchedule: DaySchedule, dayIndex: number) => {
      ...
      const daySlots = isWeekend
        ? (SCHEDULE_CONFIG.weekendEndHour - SCHEDULE_CONFIG.weekendStartHour + 1) * SCHEDULE_CONFIG.slotsPerHour
        : (SCHEDULE_CONFIG.weekdayEndHour - SCHEDULE_CONFIG.weekdayStartHour + 1) * SCHEDULE_CONFIG.slotsPerHour;
  ...
  // 스케줄 블록 그리기
  const drawScheduleBlock = (ctx: CanvasRenderingContext2D, scheduleItem: ScheduleItem, dayStartX: number, rowY: number, dayStartHour: number) => {
      ...
      <div className="bg-white shadow-sm rounded-lg border border-gray-200">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">수업 시간표</h2>
              <p className="mt-1 text-sm text-gray-600">
                Canvas 기반으로 구현된 시간표 뷰어입니다 (5분 정밀도, 1시간 격자)
              </p>
            </div>
          ...
          <div className="mt-4 text-sm text-gray-600">
            총 {schedules.length}명의 학생 | 5분 단위 정밀도 | 1시간 격자 표시
          </div>
        </div>
